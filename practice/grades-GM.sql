...
SELECT `g`.`grade` as `grade`, `g`.`grade_date` as `grade_date`,concat(`s`.name, " ", `s`.surname, " ", `s`.lastname) as fio, `group`.`name` as `name` FROM grades.students_grade as g INNER JOIN `student` as `s` on `g`.`student_id` = `s`.`id` INNER JOIN `group` on `s`.`group_id`=`group`.`id`;
